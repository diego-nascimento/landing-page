import { FooterTypes } from "../../Container/types"

const mockedFooterProps: FooterTypes = {
    logoUrl:  'data:image/png;base64,',
    sections: [
        
        {
            id: '123',
            title: 'teste',
            itens: [{
                label: 'item 1',
                url: 'asny',
                id: '12312'
            }, {
                label: 'item 1',
                url: 'asny',
                id: '12312312'
            }, {
                label: 'item 1',
                url: 'asny',
                id: '12123312'
            }, {
                label: 'item 1',
                url: 'asny',
                id: '12312313112'
            }]

        },{
            id: '123',
            title: 'teste',
            itens: [{
                label: 'item 1',
                url: 'asny',
                id: '12312'
            }, {
                label: 'item 1',
                url: 'asny',
                id: '12312312'
            }, {
                label: 'item 1',
                url: 'asny',
                id: '12123312'
            }, {
                label: 'item 1',
                url: 'asny',
                id: '12312313112'
            }]

        },
        {
            id: '123',
            title: 'teste',
            itens: [{
                label: 'item 1',
                url: 'asny',
                id: '12312'
            }, {
                label: 'item 1',
                url: 'asny',
                id: '12312312'
            }, {
                label: 'item 1',
                url: 'asny',
                id: '12123312'
            }, {
                label: 'item 1',
                url: 'asny',
                id: '12312313112'
            }]

        },
        {
            id: '123',
            title: 'teste',
            itens: [{
                label: 'item 1',
                url: 'asny',
                id: '12312'
            }, {
                label: 'item 1',
                url: 'asny',
                id: '12312312'
            }, {
                label: 'item 1',
                url: 'asny',
                id: '12123312'
            }, {
                label: 'item 1',
                url: 'asny',
                id: '12312313112'
            }]

        },
        {
            id: '123',
            title: 'teste',
            itens: [{
                label: 'item 1',
                url: 'asny',
                id: '12312'
            }, {
                label: 'item 1',
                url: 'asny',
                id: '12312312'
            }, {
                label: 'item 1',
                url: 'asny',
                id: '12123312'
            }, {
                label: 'item 1',
                url: 'asny',
                id: '12312313112'
            }]

        },
        {
            id: '123',
            title: 'teste',
            itens: [{
                label: 'item 1',
                url: 'asny',
                id: '12312'
            }, {
                label: 'item 1',
                url: 'asny',
                id: '12312312'
            }, {
                label: 'item 1',
                url: 'asny',
                id: '12123312'
            }, {
                label: 'item 1',
                url: 'asny',
                id: '12312313112'
            }]
        }, 
    ],
    socials: [{
        card: "facebook",
        url: 'facebook.com/diego-nascimento',
        numberOfFollows: 10000
    }, 
    {
        card: "youtube",
        url: 'youtube.com/diego-nascimento',
        numberOfFollows: 2051151
    },
    { 
        card: "linkedin",
        url: 'instagram.com/diego-nascimento',
        numberOfFollows: 1565156
    },{
        card: "instagram",
        url: 'instagram.com/diego-nascimento',
        numberOfFollows: 20551
    },{
        card: "twitter",
        url: 'twitter.com/diego-nascimento',
        numberOfFollows: 20551
    }]
}

export const footerService = async (): Promise<FooterTypes> => {
    return Promise.resolve(mockedFooterProps)
}